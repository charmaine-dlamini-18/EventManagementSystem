                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="flex items-center gap-3 p-3 bg-purple-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ $event->formatted_date }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ $event->date_range }}</p>
                        </div>
                    </div>
                ...
                @if($event->capacity)
                    <div class="mb-6">
                        ...
                        <p class="text-sm text-gray-500">
                            Capacity: {{ $event->capacity - $spotsRemaining }}/{{ $event->capacity }} spots filled
                            @if($spotsRemaining === 0)
                                <span class="ml-2 text-red-600 font-medium">— Full</span>
                            @else
                                <span class="ml-2 text-green-600 font-medium">— {{ $spotsRemaining }} spots left</span>
                            @endif
                        </p>
                    </div>
                @endif
            ...
            @auth
                @if($event->status === 'published')
                    ...
                    <div class="px-6 py-5 border-t border-gray-100 bg-gray-50">
                        @if($isOrganizer)
                            <p class="text-sm font-medium text-green-700">You are the organizer of this event.</p>
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div>
                                    ...
                                    <span class="inline-flex mt-1 px-2.5 py-1 rounded-full text-xs font-medium
                                        @if($userRegistration->status === 'confirmed') bg-green-100 text-green-700
                                        @elseif($userRegistration->status === 'pending') bg-amber-100 text-amber-700
                                        @else bg-red-100 text-red-700 @endif">
                        ...
                        @else
                            <p class="text-sm text-red-600 font-medium">This event is at full capacity.</p>
                        @endif
                    </div>
                @endif
            @else
                <div class="px-6 py-5 border-t border-gray-100 bg-gray-50">
                    <p class="text-sm text-gray-600">
                        Please <a href="{{ route('login') }}" class="font-medium text-green-600 hover:text-green-800">login</a>
                        or <a href="{{ route('register') }}" class="font-medium text-green-600 hover:text-green-800">register</a> to sign up for this event.
                    </p>
                </div>
            @endauth
                                ...
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                                    <div class="flex items-center gap-3">
                                        <div class="w-9 h-9 bg-green-100 rounded-full flex items-center justify-center">
                                            <span class="text-sm font-bold text-green-700">{{ $registration->user->initials }}</span>
                                        </div>
                                        <div>
                                            <p class="font-medium text-gray-900 text-sm">{{ $registration->user->name }}</p>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($registration->is_pending)
                                            <form method="POST" action="{{ route('registrations.approve', $registration) }}">
                                                @csrf
                                                <button type="submit" class="px-2.5 py-1 text-xs font-medium text-green-700 bg-green-50 rounded-lg hover:bg-green-100">
                                                    Approve
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('registrations.decline', $registration) }}">
                                                @csrf
                                                <button type="submit" class="px-2.5 py-1 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100">
                                                    Decline
                                                </button>
                                            </form>
                                        @endif
